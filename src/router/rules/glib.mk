...
#	echo ############################################################
#	./configure --enable-static --disable-shared --host=$(ARCH)-linux \
#	CC="ccache $(CC)" CFLAGS="$(COPTS) -std=gnu89 -D_GNU_SOURCE -fPIC -Drpl_malloc=malloc" \
...
#	$(MAKE) -C glib20/libffi clean all
	...
	echo ############################################################
	cd glib20/libiconv && aclocal && ./configure --enable-shared --enable-static --host=$(ARCH)-linux \
	CC="ccache $(CC)" CFLAGS="$(COPTS) -std=gnu89 $(MIPS16_OPT)  -D_GNU_SOURCE -fPIC -Drpl_malloc=malloc"
	echo 
	echo ############################################################
	echo # BUILD LIBICONV                                           #
	echo ############################################################
	...
	echo ############################################################
	cd glib20/gettext && aclocal && ./configure --enable-shared --disable-static --disable-openmp --host=$(ARCH)-linux \
	CC="ccache $(CC)" LDFLAGS="$(COPTS) -std=gnu89 $(MIPS16_OPT) -D_GNU_SOURCE -fPIC -Drpl_malloc=malloc " \
	CFLAGS="$(COPTS)  $(MIPS16_OPT)  -D_GNU_SOURCE -fPIC -Drpl_malloc=malloc -I$(TOP)/glib20/libiconv/include" \
	CXXFLAGS="$(COPTS)  $(MIPS16_OPT) -D_GNU_SOURCE -fPIC -Drpl_malloc=malloc -I$(TOP)/glib20/libiconv/include"
	echo 
	echo ############################################################
	echo # BUILD GETTEXT                                            #
	echo ############################################################
	...
	echo ############################################################
	cd glib20/libglib && ./configure --enable-shared --disable-static --disable-fam --enable-debug=no --disable-selinux --disable-man --host=$(ARCH)-linux --with-libiconv=gnu --disable-modular-tests --disable-libmount --with-pcre=internal \
